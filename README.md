#  SAUNA-CHAT 
このアプリケーションは大阪サウナ限定の情報交流サイトになります。  
...
## 目指した課題解決	
サウナの素晴らしさに気がついてほしい。  
サウナを若者のトレンドにしたい。飲み会も楽しくて良いですが、  
サウナに行き自分自身としっかりと向き合い、  
いい仕事ができる身体やメンタルを整えるのにサウナは最適です。  
...
・チャット機能もログインなしで非同期でコメントをすることができます。  
  
・新規登録またはゲストログインするとサウナ情報を投稿可能です。  
また、ログインユーザーのみ投稿ページにコメントといいねボタンを押すことが出来、連絡を取ることができます。  

## 工夫したポイント
・モバイルファーストでの実装  
サイトの閲覧はモバイル端末が圧倒的に多いという点からモバイルファーストでの実装を行いました。  
iPhone5/SE(320x568)からiPadPro(1024x1366)までビュー崩れが無い実装をすることができています。
...
Slackをイメージし実装を行いました。改善の余地はまだまだありますが本番環境でも問題なく実装できています。
...
Ajax非同期通信にする。  
（実装は完了したが、ページを変更するともっと見るボタンが固まってしまったので完全ではない。履歴を見ると送信するときにフォーマットがJSになっているのでJqueryが固まってしまったと仮説を立てており、JSのコードに書き換えることで解決できる判断し実装中。）
...
（表示を変更するともっと見るボタンが固まってしまいます。現時点での課題です。）  
